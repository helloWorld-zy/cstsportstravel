...
**Purpose**: Close remaining gaps between spec/plan intent and current codebase state. Generated by `/speckit-converge` on 2026-06-28.

**Summary**: 10 findings — 2 CRITICAL (payment gateways), 4 HIGH (password policy, lockout, addons API, homepage config, order confirm), 2 MEDIUM (role delete, SMS integration), 1 LOW (WeChat OAuth web). All 157 prior tasks (T001–T157) marked complete.

- [x] T158 [CRITICAL] Wire up real Alipay SDK integration in `internal/payment/service/alipay.go` — replace stub `CreatePayment` with `smartwalle/alipay/v3` SDK calls (page pay + wap pay), implement real `VerifyNotification` signature verification, implement `QueryOrder` against Alipay API per FR-022 and PRD §5.1.1
- [x] T159 [CRITICAL] Wire up real WeChat Pay SDK integration in `internal/payment/service/wechat.go` — replace stub `CreatePayment` with `wechatpay-go` SDK calls (Native QR + JSAPI + Mini Program), implement real `VerifyNotification` signature verification, implement `QueryOrder` per FR-023 and PRD §5.1.2
- [x] T160 Implement admin password policy enforcement in `internal/admin/handler/auth.go` and `internal/admin/service/rbac.go` — validate password complexity on create/change (8+ chars, ≥3 of 4 types), enforce 90-day expiry with 14-day warning, force password change when `must_change_password` flag is set per FR-005
- [x] T161 Implement admin login failure tracking in `internal/admin/handler/auth.go` — increment `LoginFailCount` on each failed password attempt, set `LockedUntil` to 15 minutes after 5 consecutive failures, reset counter on successful login per FR-006
- [x] T162 Connect addon selection to backend API in `web/components/booking/AddonStep.vue` — replace hardcoded sample data with API call to fetch available addons for the selected product/departure, update price on selection per FR-029
- [x] T163 Connect HomepageConfig to backend APIs in `admin-web/src/views/config/HomepageConfig.vue` — replace static arrays with `$fetch` calls to banner CRUD API (`/api/v1/admin/banners`) and destination API (`/api/v1/destinations`), wire image upload per FR-012/SC-009
- [x] T164 Implement order confirm endpoint in `internal/order/handler/order.go` — add `POST /api/v1/orders/:id/confirm` handler that validates order ownership and status (completed/in_travel), transitions to `completed` state, creates status log per FR-017
- [x] T165 Implement role deletion endpoint in `internal/admin/handler/rbac.go` and `internal/admin/service/rbac.go` — add `DELETE /api/v1/admin/roles/:id` with soft-delete, prevent deletion of system roles, check no active users are assigned per US7/AC2
- [x] T166 Integrate Alibaba Cloud SMS SDK in `internal/user/service/sms.go` — replace `sendViaProvider` stub with real `dysmsapi` SDK call using credentials from config, handle delivery status and retry per FR-001
- [x] T167 Implement WeChat OAuth redirect on web login page in `web/pages/auth/login.vue` — replace info message stub with redirect to WeChat OAuth URL, handle callback with code exchange per FR-002

---

## Phase 14: Convergence

**Purpose**: Close remaining gap from Phase 13 — T159 WeChat Pay SDK integration was only partially implemented. Generated by `/speckit-converge` on 2026-06-28.

**Summary**: 1 finding — 1 HIGH (WeChat Pay payment creation/verification stubs). All other 30 FRs, 9 SCs, and 7 user stories are satisfied.

- [x] T168 Wire up real WeChat Pay V3 API calls in `internal/payment/service/wechat.go` — replace `createNativePayment` placeholder with real `native.NativeApiService.Prepay()` call, replace `createJSAPIPayment` placeholder with real `jsapi.PrepayWithRequestPayment()` call, replace `VerifyNotification` stub with real `notify.NewRSANotifyHandler` signature verification, replace `QueryOrder` stub with real order query API per FR-023 and PRD §5.1.2
