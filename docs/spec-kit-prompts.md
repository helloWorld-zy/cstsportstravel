...
请实现 tasks.md 中 Phase 3（US1 用户注册与登录）的全部任务（T037-T056）。

执行前先阅读：
1. specs/001-domestic-tour-mvp/spec.md（US1 验收场景，FR-001~FR-006）
2. specs/001-domestic-tour-mvp/contracts/user-api.yaml（用户 API 端点定义）
3. specs/001-domestic-tour-mvp/data-model.md（user_account、real_name_verification、frequent_traveller 表）
4. specs/001-domestic-tour-mvp/tasks.md（仅 Phase 3 部分）
5. docs/travel_booking_system_v3.agent.final.md §4.1.1（用户注册与登录流程）
...
请实现 tasks.md 中 Phase 4（US2 产品搜索与浏览）的全部任务（T057-T073）。

执行前先阅读：
1. specs/001-domestic-tour-mvp/spec.md（US2 验收场景，FR-007~FR-013）
2. specs/001-domestic-tour-mvp/contracts/product-api.yaml（产品 API 端点定义）
3. specs/001-domestic-tour-mvp/data-model.md（product、itinerary、departure_date、price_rule、category、product_review 表）
4. specs/001-domestic-tour-mvp/tasks.md（仅 Phase 4 部分）
5. docs/travel_booking_system_v3.agent.final.md §4.2.1（产品列表与筛选）、§4.2.2（产品详情页）
...
请实现 tasks.md 中 Phase 5（US3 预订下单）的全部任务（T074-T096）。

执行前先阅读：
1. specs/001-domestic-tour-mvp/spec.md（US3 验收场景，FR-014~FR-025）
2. specs/001-domestic-tour-mvp/contracts/order-api.yaml（订单 API）
3. specs/001-domestic-tour-mvp/contracts/payment-api.yaml（支付 API）
4. specs/001-domestic-tour-mvp/data-model.md（main_order、order_traveller、payment_transaction 表）
5. specs/001-domestic-tour-mvp/research.md（库存并发控制、支付幂等性、订单自动取消方案）
6. specs/001-domestic-tour-mvp/tasks.md（仅 Phase 5 部分）
7. docs/travel_booking_system_v3.agent.final.md §4.2.3（预订流程）、§4.2.5（业务规则：单房差/儿童价/超时未付）、§5.1（支付渠道接入）、§5.2（支付模式）
...
请实现 tasks.md 中 Phase 6（US4 订单管理）的全部任务（T097-T105c）。

执行前先阅读：
1. specs/001-domestic-tour-mvp/spec.md（US4 验收场景，FR-017~FR-021）
2. specs/001-domestic-tour-mvp/contracts/order-api.yaml（退款相关端点）
3. specs/001-domestic-tour-mvp/data-model.md（refund_rule、refund_record 表，订单状态机 9 状态映射）
4. specs/001-domestic-tour-mvp/tasks.md（仅 Phase 6 部分）
5. docs/travel_booking_system_v3.agent.final.md §4.2.4（订单管理）、§4.2.5（退改规则）、§6.2.3（退改审核）、§6.2.4（退改阶梯费率表 6-6）
...
请实现 tasks.md 中 Phase 7（US5 后台产品管理）的全部任务（T106-T116）。

执行前先阅读：
1. specs/001-domestic-tour-mvp/spec.md（US5 验收场景，FR-007~FR-013）
2. specs/001-domestic-tour-mvp/contracts/admin-api.yaml（产品管理相关端点）
3. specs/001-domestic-tour-mvp/data-model.md（product 状态机 5 状态、departure_date、price_rule 表）
4. specs/001-domestic-tour-mvp/tasks.md（仅 Phase 7 部分）
5. docs/travel_booking_system_v3.agent.final.md §6.1.1（境内游产品发布）、§6.1.8（产品审核流程）、§6.1.9（价格日历管理）、§6.1.10（库存管理）
...
请实现 tasks.md 中 Phase 8（US6 后台订单管理）的全部任务（T117-T124）。

执行前先阅读：
1. specs/001-domestic-tour-mvp/spec.md（US6 验收场景，FR-020~FR-021）
2. specs/001-domestic-tour-mvp/contracts/admin-api.yaml（订单/退款管理端点）
3. specs/001-domestic-tour-mvp/tasks.md（仅 Phase 8 部分）
4. docs/travel_booking_system_v3.agent.final.md §6.2.1（订单列表与查询）、§6.2.3（退改审核流程）、§6.2.4（退改规则配置）
...
请实现 tasks.md 中 Phase 9（US7 基础权限管理）的全部任务（T125-T133b）。

执行前先阅读：
1. specs/001-domestic-tour-mvp/spec.md（US7 验收场景，FR-026~FR-028, FR-030）
2. specs/001-domestic-tour-mvp/contracts/admin-api.yaml（用户/角色/权限/菜单端点）
3. specs/001-domestic-tour-mvp/tasks.md（仅 Phase 9 部分）
4. .specify/memory/constitution.md（Principle III MFA 要求）
...
请实现 tasks.md 中 Phase 10（前端增强）的全部任务（T134-T139）。

执行前先阅读：
1. specs/001-domestic-tour-mvp/spec.md（US2 首页相关验收场景）
2. specs/001-domestic-tour-mvp/tasks.md（仅 Phase 10 部分）
...
请实现 tasks.md 中 Phase 11（集成测试与安全加固）的全部任务（T140-T148）。

执行前先阅读：
1. specs/001-domestic-tour-mvp/quickstart.md（全部验证场景 VS1-VS8）
2. specs/001-domestic-tour-mvp/tasks.md（仅 Phase 11 部分）
3. .specify/memory/constitution.md（Principle III 安全基线逐项检查）
...
请实现 tasks.md 中 Phase 12（部署与运维）的全部任务（T149-T157）。

执行前先阅读：
1. specs/001-domestic-tour-mvp/plan.md（部署架构、CI/CD 方案）
2. specs/001-domestic-tour-mvp/tasks.md（仅 Phase 12 部分）
3. docs/travel_booking_system_v3.agent.final.md §10.4.2（监控告警，表 9-5 的 16 项指标和阈值）
